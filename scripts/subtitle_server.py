...
def _dashboard_html() -> str:
    """Add utterance-keyed translation placement to the embedded dashboard.

    The original dashboard kept only ``lastCard``.  That is unsafe for the
    multiplexed Discord service because translations can complete after a
    different speaker has finalized.  Keep the compact embedded asset while
    injecting the keyed lookup in one auditable place.
    """
    html = DASHBOARD_HTML.replace(
        "let finals = 0, latSum = 0, langs = new Set(), lastCard = null, idleTimer = null;",
        "let finals = 0, latSum = 0, langs = new Set(), lastCard = null, "
        "idleTimer = null, cards = new Map();\n"
        "  function eventKey(ev) {\n"
        "    return String(ev.stream_id ?? 'local') + ':' + "
        "String(ev.utterance_id ?? 'latest');\n"
        "  }",
    )
    html = html.replace(
        "feed.appendChild(card); lastCard = card; stick(feed);",
        "feed.appendChild(card); lastCard = card; cards.set(eventKey(ev), card); stick(feed);",
    )
    html = html.replace(
        "if (!lastCard) return;\n      const tr = document.createElement(\"div\");",
        "const targetCard = cards.get(eventKey(ev)) || lastCard;\n"
        "      if (!targetCard) return;\n"
        "      const tr = document.createElement(\"div\");",
    )
    html = html.replace(
        "lastCard.appendChild(tr); stick(feed);",
        "targetCard.appendChild(tr); stick(feed);",
    )
    return html
